...
- **File Tools**: `readFile`, `writeFile`, `replace`, `listDirectory`, `glob`
- **Shell Tools**: `runShellCommand`, `grepSearch`
...
실행 후 `jwlee:>` 프롬프트가 나타나면 명령어 입력을 대기합니다.
...
### 1. README 업데이트 에이전트
현재 `README.md`의 내용을 읽고, 사용자의 요청에 따라 내용을 개선한 뒤 승인을 거쳐 파일을 덮어씁니다.
```bash
jwlee:> update-readme "프로젝트 목표 부분을 조금 더 기술적으로 강조해서 요약해줘"
```

### 2. HITL 데모 에이전트
Human-in-the-Loop 승인 절차가 터미널 환경에서 어떻게 비동기로 동작하는지 확인하는 간단한 데모입니다.
```bash
jwlee:> demo-hitl "가상의 위험한 명령 실행"
```

### 3. 발표자료 생성 에이전트 (Legacy)
로컬 문서(RAG)를 기반으로 마크다운 형식의 프레젠테이션 슬라이드를 자동 생성하고, 특정 페이지만 정밀하게 수정하는 고급 에이전트입니다. (상세 내용은 내부 문서 참고)
