...
- **단일 패스 분석**: 각 Diff 번들마다 LLM을 한 번만 호출하여 `Style Review`와 `Architecture Review` 결과를 동시에 생성합니다. 기존의 이중 호출 대비 토큰 사용량을 줄였습니다.
- **PR Overview 반영**: Bitbucket PR 설명의 `Overview` 섹션을 우선 읽고, 없으면 본문/제목을 fallback으로 사용하여 리뷰 맥락에 반영합니다.
- **분석 이원화 및 RAG 정책**:
  - **Style Review**: RAG를 사용하지 않고 오직 스타일 가이드 문서와 코드만으로 분석합니다.
  - **Architecture Review**: 같은 호출 안에서 Confluence 아키텍처 가이드와 PR 맥락을 함께 보며, 추가 정보가 필요한 경우에만 standards/manuals RAG를 참조합니다.
- **분리 점수 산정**: `Style Score`, `Architecture Score`, `Overall Score`를 분리하여 출력합니다.
- **GOOD 코멘트 절제**: 긍정 코멘트는 선택 사항이며, 최종 요약에는 최대 2개까지만 하이라이트합니다.
- **테스트 PR 완화**: 테스트 중심 PR은 기준을 한 단계 완화하고, 대규모 프로덕션 변경에서만 테스트 부족을 강하게 지적합니다.
...
- **Arch 분석**: SOLID 원칙, 계층 분리, 아키텍처 가이드라인 준수, PR Overview 대비 요구사항 적합성.
- **테스트 판단**:
  - 테스트 전용/테스트 중심 PR은 핵심 시나리오 검증 여부를 우선 봅니다.
  - 대규모 기능 추가나 구조 변경 PR은 회귀 위험 대비 테스트 근거를 더 엄격하게 봅니다.

## 4. 출력 결과
- Bitbucket PR에 라인별 코멘트 및 전역 요약 리포트가 게시됩니다.
- CLI 화면과 전역 요약에 `Overall`, `Style`, `Architecture` 점수가 함께 출력됩니다.
